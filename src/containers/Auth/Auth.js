    ...
    render() {
        ...
        return (
            <div className={classes.Auth}>
                {authRedirect}
                {errorMessage}
                <form onSubmit={this.onSumbitHanle}>
                    {form}
                    <Button btnType="Success">SUBMIT</Button>
                </form>
                <Button
                    clicked={this.switchAuthMode}
                    btnType="Danger">Go To {this.state.signUp ? 'SIGNIN' : 'SIGNUP'}</Button>
            </div>
        );
    }
}
...
const mapStateToProps = state => {
    return {
        ...
        buildingSandwich: state.sandwichMaker.building,
        authRedirectPath: state.auth.authRedirectPathh
    }
}
